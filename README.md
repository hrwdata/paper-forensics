...
`paper-forensics` is a TeX-first manuscript review application for sentence-level overlap and AI-assisted tell evidence. It is built for inspectable review, not for making definitive claims about plagiarism, AI authorship, or misconduct.
...
- `ai_rhetoric_risk_score`: estimated risk from formulaic, boilerplate, or AI-assisted tell signals
...
- **Problem**: help reviewers inspect sentence-level overlap and AI-assisted tell signals in TeX manuscripts.
...
- `paper-forensics audit ...` writes `audit.json`, `audit.csv`, and a single-file `report.html`
- `paper-forensics app` launches a local upload/analyze/review application for `.tex` files and TeX project archives

The review UI is designed for editors, manuscript reviewers, and research-integrity workflows:

- three-pane workstation layout
- evidence-first sentence inspector
- restrained overlap and AI-assisted tell highlighting
...
- AI-assisted tell evidence with phrase triggers, category summaries, and feature breakdowns
...
- AI-assisted tell threshold
- flagged-only review
- only sentences with external matches
- only sentences with AI-assisted tell triggers
- section filter
- free-text search
...
- AI-assisted tell scoring from curated phrase rules, semantic thinness heuristics, and local style shift
- static review report and local app mode
...
- mean AI-rhetoric risk: `0.2450`
- max plagiarism risk: `0.9250`
- max AI-rhetoric risk: `0.5078`
- sentences with plagiarism risk `>= 0.6`: `2`
- sentences with AI-rhetoric risk `>= 0.6`: `0`

Observed AI-assisted tell category totals in the same sample:

- `balanced_summary`: `2`
- `empty_transition`: `2`
- `framework_boilerplate`: `1`
- `low_specificity`: `1`
...
- **Template-heavy prose**: formulaic academic writing may trigger AI-assisted tell rules even when human-authored.
...
- All scores are heuristic; this tool is a review aid, not a definitive detector.
